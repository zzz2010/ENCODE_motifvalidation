...
for r in $(echo $BACKREGIONS); do 
	...
	if [ ! -f $DIR/compare/$(rname $r)/regions_sizes.txt ]; then
		echo "making $DIR/compare/$(rname $r)/regions_sizes.txt" 
		# 1) region
...
if [ ! -z "$MOTIFFILE" ]; then
	...
	if [ ! -d $OPTMMDIR/${MATCHMASKS/ */}/matches ]; then 
		...
		FindOptMMParams.sh -N $NUMNODES -M -- -m $DIR/motifs.txt -r $DIR/compare/$(rname $r)/bregions.txt.gz -d $OPTMMDIR -S $HSTR -E $OPTMMDIR -z $CONFIDZ -h 3 -o $ORG -k "$MATCHMASKS" -f "$MATCHESNAMES" -F "$([ "$LOWMODE" = 1 ] && echo "" || echo "0 1")"
	fi
else
	...
	if [ -z "$MATCHMASKS" ]; then
		MATCHMASKS=$(
			for K in $(echo $KMERSD); do 
				find $OPTMMDIR -maxdepth 2 -name matches | awk -F'/' '{print $(NF-1)}'
	...
	if [ -z "$MATCHESNAMES" ]; then
		MATCHESNAMES=$(
			for K in $(echo $KMERSD); do for M in $(echo $MATCHMASKS); do
				if [ -d $OPTMMDIR/$M/matches ]; then 
					find $OPTMMDIR/$M/matches -type f 
				fi
...
if [ "$LOWMODE" = 0 ]; then
	...
	for r in $(echo $BACKREGIONS); do  for K in $(echo $KMERSD); do for M in $(echo $MATCHMASKS); do  for n in $(echo $MATCHESNAMES); do
		if [ -f $OPTMMDIR/$M/matches/$n.gz ]; then 
			mkdir -p $DIR/compare/$(rname $r)/$M/$n
			if [ ! -f $DIR/compare/$(rname $r)/$M/$n/match-counts.txt.gz ]; then 
				cat <<-EOF
				grep-overlap -t$HSTR -r1 $OPTMMDIR/$M/matches/$n.gz $DIR/compare/$(rname $r)/regions.txt.gz | pcut -t '|' -T ' ' -f 2.1 1.1 + | gzip -c > $DIR/compare/$(rname $r)/$M/$n/match-counts.txt.gz

				EOF
			fi

			if [ ! -f $DIR/compare/$(rname $r)/$M/$n/match-region-counts.txt.gz ]; then 
				if [ $PRODUCERCF = 1 ]; then 
					# NOTE: unique identifier in 6th column ensures we don't miss duplicates
					cat <<-EOF
					grep-overlap -t$HSTR -r1 -o $OPTMMDIR/$M/matches/$n.gz $DIR/compare/$(rname $r)/bregions.txt.gz | grep-overlap -t$HSTR -r2 $DIR/regions.txt.gz - | awk -F'|' '
						...
						' | gzip -c > $DIR/compare/$(rname $r)/$M/$n/match-region-counts.txt.gz

					EOF
				else
					echo -n | gzip -c > $DIR/compare/$(rname $r)/$M/$n/match-region-counts.txt.gz
				fi
			fi	
		fi
	...
	for r in $(echo $BACKREGIONS); do for K in $(echo $KMERSD); do for M in $(echo $MATCHMASKS); do for n in $(echo $MATCHESNAMES); do
		...
		if [ ! -f $DIR/compare/$(rname $r)/$M/$n/match-counts.txt.gz ]; then 
			rmc -N instcompare -v $DIR/compare.rmc -c "extract-mfa -k 1 $(GetOrgVars.sh $ORG ALIGNFILE) < %s | motif-match -n 1 -m $DIR/optmm/motifs-toscan.txt | grep-overlap -t$HSTR -r1 - $DIR/compare/$(rname $r)/regions.txt.gz  | pcut -t '|' -T ' ' -f 2.1 1.1 +" -i $DIR/optmm/regions.txt -e - -n $NUMNODES | pcut -f 1 2 +3 -T ' ' | awk '
				ARGIND==1{
					X[$1]=$1
					for (i=2; i<=NF; i++)
						X[$i] = $1 "_C" (i-1)
				...
				ARGIND==2{
					print $1, X[$2], $3
				}' $DIR/optmm/Xmer/final-controls.txt - | gzip -c > $DIR/compare/$(rname $r)/$M/$n/match-counts.txt.gz
		fi
