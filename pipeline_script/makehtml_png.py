...
def motifPosRankPlot(bestmotif):
	...
	outfn=resultDir+"/plots/"+bestmotif+".png"
...
def MotifClustRow(motifclust):
	...
	imageFn=dataPath+"/motifDist/"+motifclust+".png"
...
def TFImageTRs(tf):
	imageFiles=glob.glob(dataPath+"/motifDist/"+tf+"_*.png")
	outstr=""
	for imageFn in imageFiles:
		motifclust=os.path.basename(imageFn).replace(".png","")	
		if motifclust not in motifclust_bestmotif:
			continue	
		outstr+=MotifClustRow(motifclust)
	return outstr

def motifLogo(motif):
	logourl="http://www.broadinstitute.org/~zzhang/logos2/"+motif+".png"
        ret_str="<div  style=\"width:218px; overflow-x:hidden;\"><embed  src="+logourl+" width=218px  height=100px  onload=lzld(this)></embed>best motif: "+motif+"</div>\n"
	return ret_str
...
def MotifClustTable(lines):
	header="MotifClusterId       IsUseless      PositionBiasZscore      PeakRankBiasZscore      GlobalEnrichmentZscore  CombineAvgScore EnrichmentRank(total:283)       bestMotif"
...
for line in open(tfresultFn):
	...
	prob=float(comps[2].replace("!",""))
	tf_accprb[comps[0]]=prob
