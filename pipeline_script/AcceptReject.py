...
for tf in tfBestScore:
	...
	if tf not in tf_validatability: #that mean not in training data
		continue
	probstr=str(odd2prob(logOdds))
	if tfBestMotif[tf] not in modelparameter_z1:
		probstr+="!"
	outf.write(Arr2str([tf,float(logOdds),probstr])+"\t"+tfBestMotif[tf]+"\t"+(tfBestMotifScore[tf])+"\t"+tf_validatability[tf]+"\n")
outf.close()
